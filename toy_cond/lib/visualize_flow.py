...
def plt_flow(c, prior_logdensity, transform, ax, npts=100, title="$q(x)$", device="cpu", args=None):
    """
    Args:
    ...
    z = torch.zeros([zz.shape[0], args.dist_dims], requires_grad=True).type(torch.float32).to(device)
    z[:, :zz.shape[1]] = torch.tensor(zz).to(z)
    logqz = prior_logdensity(z)
    logqz = torch.sum(logqz, dim=1)[:, None]
    z, logqz = transform(z, c, logqz)
    logqz = torch.sum(logqz, dim=1)[:, None]
...
def plt_flow_density(prior_logdensity, inverse_transform, ax, npts=100, memory=100, title="$q(x)$", device="cpu", args=None):
    side = np.linspace(LOW, HIGH, npts)
    xx, yy = np.meshgrid(side, side)
    x = np.hstack([xx.reshape(-1, 1), yy.reshape(-1, 1)])

    x = torch.from_numpy(x).type(torch.float32).to(device)
    zeros = torch.zeros(x.shape[0], args.cond_dims).to(x)
...
def plt_flow_samples(c, prior_sample, n_samples, transform, ax, npts=100, memory=100, title="$x ~ q(x)$", device="cpu", args=None):
    z = prior_sample(n_samples, args.dist_dims).type(torch.float32).to(device)
    zk = []
    inds = torch.arange(0, z.shape[0]).to(torch.int64)
    for ii in torch.split(inds, int(memory**2)):
        zk.append(transform(z[ii], c))
    zk = torch.cat(zk, 0).cpu().numpy()

    ax.scatter(zk[:,0], zk[:,1], s=0.1, c='w', marker="o")
...
def visualize_transform(
    potential_or_samples, c, prior_sample, prior_density, transform=None, samples=True, npts=100,
    memory=100, device="cpu", args=None
):
    ...
    plt_flow(c, prior_density, transform, ax, npts=npts, device=device, args=args)

    ax = plt.subplot(1, 3, 3, aspect="equal")
    if transform is not None:
        n_samples = potential_or_samples.shape[0]
        plt_flow_samples(c, prior_sample, n_samples, transform, ax, npts=npts, memory=memory, device=device, args=args)
