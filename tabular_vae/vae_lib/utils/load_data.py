...
def load_cifar10(args, **kwargs):
    ...
    train_set = dset.CIFAR10(
        root="./data", train=True, transform=T.Compose([
            T.Resize(im_size),
            T.RandomHorizontalFlip(),
            T.ToTensor(),
        ]), download=True
    )
    trans = lambda im_size: T.Compose([T.Resize(im_size), T.ToTensor()])
...
def load_dataset(args, **kwargs):
    ...
    if args.dataset == 'mnist':
        ...
        train_loader, val_loader, test_loader, args = load_cifar10(args)
    else:
