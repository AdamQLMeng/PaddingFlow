...
def get_hidden_dims(args):
    return tuple(map(int, args.dims.split("-"))) + (args.z_size+args.padding_dim,)
...
class CNFVAE(VAE):
    ...
    def __init__(self, args):
        ...
        # CNF model
        self.cnf = build_model_tabular(args, args.z_size+args.padding_dim)
...
class AmortizedCNFVAE(VAE):
    ...
    def __init__(self, args):
        ...
        self.odefuncs = nn.ModuleList([
            construct_amortized_odefunc(args, args.z_size+args.padding_dim, self.amortization_type) for _ in range(args.num_blocks)
        ])
    ...
    def forward(self, x):
        ...
        z_denoise = self.denoise(z)
        x_mean = self.decode(z_denoise)
