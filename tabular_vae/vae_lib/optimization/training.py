...
def train(epoch, train_loader, model, opt, args, logger):
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        loss, rec, kl, bpd = calculate_loss(x_mean, data, z_mu, z_var, z0, zk, ldj, args,
                                                         beta=beta)
        ...
        if batch_idx % args.log_interval == 0:
            ...
            log_msg = (
                'Epoch {:3d} [{:5d}/{:5d} ({:2.0f}%)] | Time {:.3f} | Loss {:11.6f} | Bits/dim {:8.6f}'
                ' | Rec {:11.6f} | KL {:11.6f}'.format(
                    epoch, num_data, len(train_loader.sampler), perc, batch_time, loss.item(), bpd, rec, kl
                )
            )
            if 'cnf' in args.flow:
    ...
    if args.input_type == 'binary':
        ...
        logger.info(
            f'====> Epoch: {epoch} Average train loss: {train_loss.sum() / len(train_loader)}, '
            f'average bpd: {train_bpd.sum() / len(train_loader)}'
        )
...
def evaluate(data_loader, model, args, logger, testing=False, epoch=0):
    ...
    with tqdm.tqdm(total=len(data_loader), desc="[Evaluate]: ") as pbar:
        for data, _ in data_loader:
            ...
            with torch.no_grad():
                ...
                batch_loss, batch_rec, batch_kl, batch_bpd = calculate_loss(x_mean, data, z_mu, z_var, z0, zk, ldj, args)
    ...
    if testing:
        logger.info(f'====> Test set loss: {loss}; bpd: {bpd}')
    ...
    if not testing:
        ...
        return log_likelihood, nll_bpd, loss, bpd, rec
