...
def plt_samples(samples, ax, npts=100, title="$x ~ p(x)$"):
    ax.scatter(samples[:, 0], samples[:, 1], s=0.1, c='w', marker="o")
    ax.set_facecolor('midnightblue')
    ax.set_xlim(LOW, HIGH)
    ax.set_ylim(LOW, HIGH)
...
def plt_latent(samples, inverse_transform, ax, device, title="$z ~ q(z)$"):
    zero = torch.zeros(samples.shape[0], 1).to(device)
    z = inverse_transform(samples.to(zero), zero)[0].cpu().detach().numpy()
    ax.scatter(z[:, 0], z[:, 1], s=0.1, c='b', marker="o")
    ax.set_xlim(LOW, HIGH)
    ax.set_ylim(LOW, HIGH)
...
def visualize_transform(
    ...
    plt_latent(potential_or_samples, inverse_transform, ax, device)
